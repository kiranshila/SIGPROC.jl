...
function dedisperse(dyn_spec::Filterbank, DM)
    ...
    for freq in freqs
        shift_t = Δt(DM, freq, f_max)
        shift_samp = Int(shift_t ÷ tsamp)
        dedisp[Freq(At(freq))] .= dyn_spec.data[Ti=(1 + shift_samp):(samp_max + shift_samp),
                                                Freq(At(freq))]
    end

    return Filterbank(dedisp,dyn_spec.headers)
end

function dedisperse_power(dyn_spec, DM)
    tsamp = dyn_spec.headers["tsamp"]
    time = dyn_spec.data.dims[1]
    freqs = dyn_spec.data.dims[2]
    f_min, f_max = extrema(freqs)
    ...
    for freq in freqs
        shift_t = Δt(DM, freq, f_max)
        shift_samp = Int(shift_t ÷ tsamp)
        s += @view dyn_spec.data[Ti=(1 + shift_samp):(samp_max + shift_samp),Freq(At(freq))]                  
    end
    return s ./ samp_max
end
